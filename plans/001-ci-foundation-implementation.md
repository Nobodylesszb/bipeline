...
- **Registry 使用 Zot 2.1.18、固定 NodePort、仓库级 ACL 和本地 CA TLS** `(updated before implementation)`：`ci-pusher` 只写 `bipeline/**`，`k8s-puller` 只读，K3s/containerd 与 BuildKit 显式信任该 CA。Rejected: HTTP insecure registry——会让开发环境形成无法迁移的安全例外；Rejected: Harbor——超出基础闭环资源需求；Rejected: 原生 Distribution——需要额外系统才能提供当前要求的身份隔离和 UI。Based on: `docs/decisions/001-ci-toolchain-baseline.md:51-65`。
